...
class KikClient:
    ...
    def check_usernames_uniqueness(self, username_list):
        """
        Checks the uniqueness of multiple usernames.
        Results are returned in the on_username_uniqueness_received() callback for each username.

        :param username_list: A list of usernames to check for their existence
        """
        for username in username_list:
            self.logger.info(f"Checking for Uniqueness of username '{username}'")
        self._send_xmpp_element(sign_up.CheckUsernameUniquenessRequest(username))
